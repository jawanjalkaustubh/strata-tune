...
import { DEFAULT_LAYOUT, isDefaultLayout, movePanel, parseLayout, placeLayout, rowsOf, snapSpan, type Layout } from '../src/components/monitor/layout';
...
describe('Monitor layout (phase1-polish item 3)', () => {
  it('parseLayout round-trips what the settings carry and drops what does not parse', () => {
    const saved = [{ id: 'gpu', span: 3, rows: 2 }, { id: 'power', span: 3 }, { id: 'board', span: 6, height: 240 }, { id: 'cpu', span: 3 }];
    ...
    expect(parseLayout([{ id: 'gpu', span: 5 }, { id: 'nope', span: 3 }, { id: 'cpu', span: 2, height: 10 }, { id: 'cpu', span: 4 }])).toEqual([
      { id: 'cpu', span: 2 }, { id: 'gpu', span: 3, rows: 2 }, { id: 'power', span: 3 }, { id: 'board', span: 6 }
    ]);
  ...
  it('the default: CPU and power stack beside the two-row GPU panel, the board underneath, no hole', () => {
    expect(cell(DEFAULT_LAYOUT, 'cpu')).toEqual([0, 0, 1, 3]);
    expect(cell(DEFAULT_LAYOUT, 'gpu')).toEqual([0, 3, 2, 3]);
    expect(cell(DEFAULT_LAYOUT, 'power')).toEqual([1, 0, 1, 3]);
    expect(cell(DEFAULT_LAYOUT, 'board')).toEqual([2, 0, 1, 6]);
    expect(holes(DEFAULT_LAYOUT)).toBe(0);
    expect(rowsOf(placeLayout(DEFAULT_LAYOUT))).toEqual({ rows: 3, lastRow: new Set(['board']) });
  });
  ...
  it('the integration run\'s moved state (gpu, power, board, cpu) packs the CPU panel back beside the GPU instead of leaving a hole', () => {
    const moved = movePanel(DEFAULT_LAYOUT, 'cpu', 'board');
  ...
  it('movePanel is sortable: the dragged panel takes the target\'s slot, nothing else changes', () => {
    expect(movePanel(DEFAULT_LAYOUT, 'board', 'cpu').map((p) => p.id)).toEqual(['board', 'cpu', 'gpu', 'power']);
