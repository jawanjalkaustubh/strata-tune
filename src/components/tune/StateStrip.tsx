...
/**
 * The rollback state machine as one line (plan 16): the stage, the baseline the
 * collector falls back to (tagged with where it came from), the candidate on the card
 * now, the values a crash revert undid, and the warning that another OC tool is
 * running. Absent facts collapse; nothing here animates.
 */
export const StateStrip: React.FC<Props> = ({ status, reason, refusal }) => {
  if (!status) {
    return (
      <div className="rounded-md border border-studio-border bg-studio-panel px-3 h-10 flex items-center gap-3 min-w-0">
        <Pill tone="idle">No state</Pill>
        <span className="text-mini text-studio-muted truncate">{reason ?? 'Waiting for the collector'}</span>
      </div>
    );
  }
  ...
  const baselineTag = status.baseline ? (status.nvapi.deltas ? 'read from the driver' : 'from the state file') : undefined;
  return (
    <div className="rounded-md border border-studio-border bg-studio-panel px-3 py-2 flex flex-wrap items-center gap-x-5 gap-y-1.5 min-w-0">
      ...
      {status.baseline ? (
        ...
        <span className="text-mini text-studio-muted" title={status.nvapi.reason ?? undefined}>
      ...
      {tool && (
        <span className="inline-flex items-center gap-1.5 text-mini text-amber-300" title={refusal ?? undefined}>
          <AlertTriangle size={13} /> {tool} is running — close it for the run
        </span>
      )}
      {status.enabled && !status.revertTaskRegistered && (
        <span className="text-micro text-amber-300/80" title={`${status.revertTaskProblem ?? 'schtasks could not register it'}. A hard hang would be reverted only at the next app start`}>
          revert-at-logon task not registered
        </span>
      )}
    </div>
  );
