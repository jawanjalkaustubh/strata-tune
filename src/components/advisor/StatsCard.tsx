      ...
      {p.spec && t ? (
        <div className="space-y-2">
          ...
          <div className="text-[10px] text-studio-subtle flex flex-wrap gap-x-3">
            <span>
              Reference design figures <Tag kind="spec" />: <Link href={p.spec.tpuUrl}>TechPowerUp GPU database</Link> · <Link href={p.spec.source}>vendor page</Link>. A board-partner card runs above them.
            </span>
          </div>
        </div>
      ) : (
