...
**Machines the build has run on** (one row per real machine; the class rows above are the
design, these are the evidence). The dev box is the high-end row. The first laptop was tested
on 2026-09-19 from the shipped 0.1.0 zip, the first machine after the dev box and the first
AMD GPU (`docs/screenshots/laptop/`, fixture `tests/fixtures/classes/gaming-laptop-amd.machine.json`):

| Machine | Class | CPU · GPU · RAM · board · Windows · drivers | What 0.1.0 did on it, and what changed |
|---|---|---|---|
| ASUS ROG Zephyrus G14 GA402RJ (2022), BIOS 318 | gaming laptop, AMD/AMD hybrid | Ryzen 9 6900HS (8c/16t, Rembrandt) with Radeon Graphics (iGPU, 512 MiB) · Radeon RX 6700S 8 GB (dGPU, asleep at idle; the iGPU drives the desktop) · 16 GB DDR5-4800 (2 × Samsung M425R1GB4BB0-CQK0D SO-DIMM) · Micron 2450 1 TB NVMe · Windows 11 Home 23H2 build 22631.6199 · Adrenalin 31.0.12024.2005 (2022-09-06) · PawnIO not installed at first · account not in *Performance Log Users* | **Worked:** launch, disclaimer, UAC, handshake (warm at t+2.2 s), both AMD GPUs' library sensors, RAM, NVMe SMART, PDH, the non-GPU audit rules, the board panel's one-line absence, the model list, the bench on the right adapter (RX 6700S, 197 GB/s of 224, 5.5 TFLOPS fp32), About and the serial-free system report, the no-orphan rule (collector gone 0.3 s after the window, three times). **Broke, fixed on branch `laptop-amd`:** a second audit could start while the first still ran (the run now lives outside the page); six "No NVIDIA GPU was found" audit cards (now one plain card, the GPU loads skipped); four "sensor not readable" CPU cards (now one sentence naming PawnIO, the CPU load skipped); AI Models read "no discrete GPU · RAM 16 GiB", threw away its own bench and flashed the dev box's 5090 for a second (the snapshot now lists every adapter; the RX 6700S sizes the models); the Monitor's card panel said "integrated · no discrete card" and had no battery panel (a discrete AMD card gets its VRAM, memory clock, hot spot and fan; a battery panel exists on laptops); the CPU panel was blank without PawnIO (now the sentence and the link); Capture failed with PresentMon's exit 6 after the bench had started (the page now checks *Performance Log Users* first and offers to add the account; the bench is closed when the trace is refused); the Headroom switch was offered on a card the hunt cannot touch (now the reason, no switch); the power-plan rule called an unplugged laptop's efficiency mode a FIX (battery-aware now); the Samsung SO-DIMMs were "not in the table"; turning the Headroom switch on blanked the window (React #310, a hook after an early return, a 0.1.0 bug on every machine; fixed, and a page boundary keeps the rest of the app alive). **With PawnIO 2.2.0 installed (same day):** the CPU panel draws the eight cores, Tctl and the package against the 35 W default; the 20 s all-core load read 92 °C (3 °C from Tjmax, clocks holding), 3.4 GHz effective, 37 W, and the four CPU rules judged it; the package-power card names the vendor app's mode on a laptop, not the BIOS. |
